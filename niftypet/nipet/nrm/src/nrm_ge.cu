...
/** CUDA version of niftypet.nipet.nrm.ge */
__global__ void knl_gpu_nrm_ge(float *const effsn, const float *const ceff, const int ceff_len,
                               const int r0, const int r1, const int *const txLUT_s2c,
                               const int txLUT_s2c_len,
...
void gpu_nrm_ge(float *const effsn, const size_t effsn_size, const float *const ceff,
  ...
  knl_gpu_nrm_ge<<<blcks, thrds>>>(effsn, ceff, (int)ceff_len, r0, r1, txLUT_s2c,
                                   (int)txLUT_s2c_len, tt_ssgn_thresh);
