...
//> Module Method Table
static PyMethodDef prjsig_methods[] = {
...
static struct PyModuleDef prjsig_module = {
    PyModuleDef_HEAD_INIT,
    "prjsig", //> name of module
...
PyMODINIT_FUNC PyInit_prjsig(void) {
  ...
  return PyModule_Create(&prjsig_module);
}
...
static PyObject *trnx_prj(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOOOO", &o_prjout, &o_im, &o_tv, &o_tt, &o_txLUT, &o_sigcnst))
    return NULL;
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

  PyObject *pd_log = PyDict_GetItemString(o_sigcnst, "LOG");
  Cnt.LOG = (char)PyLong_AsLong(pd_log);
  PyObject *pd_devid = PyDict_GetItemString(o_sigcnst, "DEVID");
...
static PyObject *frwd_prj(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  PyObject *pd_span = PyDict_GetItemString(o_sigcnst, "SPN");
  Cnt.SPN = (char)PyLong_AsLong(pd_span);
  PyObject *pd_rngstrt = PyDict_GetItemString(o_sigcnst, "RNG_STRT");
  Cnt.RNG_STRT = (char)PyLong_AsLong(pd_rngstrt);
  PyObject *pd_rngend = PyDict_GetItemString(o_sigcnst, "RNG_END");
...
static PyObject *back_prj(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  static const char *kwds[] = {"bimg", "sino", "txLUT", "axLUT", "subs", "cnst", "div_sino", "sync",  NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O&O&OOOO|O&b", (char **)kwds, &asPyCuVec_f,
                                   ...
                                   &o_sigcnst, &asPyCuVec_f, &o_div_sino, &SYNC))
    return NULL;
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
