...
//> Module Method Table
static PyMethodDef prjinv_methods[] = {
...
static struct PyModuleDef prjinv_module = {
    PyModuleDef_HEAD_INIT,
    "prjinv", //> name of module
...
PyMODINIT_FUNC PyInit_prjinv(void) {
  ...
  return PyModule_Create(&prjinv_module);
}
...
static PyObject *trnx_prj(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOOOO", &o_prjout, &o_im, &o_tv, &o_tt, &o_txLUT, &o_invcnst))
    return NULL;
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

  PyObject *pd_log = PyDict_GetItemString(o_invcnst, "LOG");
  Cnt.LOG = (char)PyLong_AsLong(pd_log);
  PyObject *pd_devid = PyDict_GetItemString(o_invcnst, "DEVID");
  Cnt.DEVID = (char)PyLong_AsLong(pd_devid);
  PyObject *pd_tfov2 = PyDict_GetItemString(o_invcnst, "TFOV2");
...
static PyObject *frwd_prj(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  PyObject *pd_span = PyDict_GetItemString(o_invcnst, "SPN");
  Cnt.SPN = (char)PyLong_AsLong(pd_span);
  PyObject *pd_rngstrt = PyDict_GetItemString(o_invcnst, "RNG_STRT");
  Cnt.RNG_STRT = (char)PyLong_AsLong(pd_rngstrt);
  PyObject *pd_rngend = PyDict_GetItemString(o_invcnst, "RNG_END");
  ...
  PyObject *pd_tfov2 = PyDict_GetItemString(o_invcnst, "TFOV2");
  Cnt.TFOV2 = (float)PyFloat_AsDouble(pd_tfov2);
  PyObject *pd_zoom = PyDict_GetItemString(o_invcnst, "zoom");
