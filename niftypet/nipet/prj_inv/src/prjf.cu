...
void gpu_fprj(float *d_sn, float *d_im, float *li2rng, short *li2sn, char *li2nos, short *s2c,
  ...
  // SPAN-3 for the cross sinograms, producing 4319, instead of 6400 sinograms
  else if (Cnt.SPN == 3) {
    snno = NSINOS;
    nrng_c = NRNGS;
    nil2r_c = NLI2R;
  ...
  int z_iter = (Nz+NIPET_CU_THREADS-1)/NIPET_CU_THREADS;

  for(int i=0; i<z_iter; i++){
    //============================================================================
    fprj_oblq<<<Nprj, Nz/z_iter>>>(d_sn, d_im, d_tt, d_tv, d_subs, snno, Cnt.SPN, att, zoff, nil2r_c);
    HANDLE_ERROR(cudaGetLastError());
    //============================================================================
