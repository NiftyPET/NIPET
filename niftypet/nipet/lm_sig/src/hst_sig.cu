...
__constant__ short c_r2s[NRNGS * NRNGS];
  ...
  for (int i = i_start; i < i_stop; i++) {
    if ((itag >= 0) && (itag < (tstop - tstart) / ITIME)) {
      if ((lm[i].x & 0x7) == 5) {
        ...
        int aw = c2s[(lm[i].y >> 6) + (lm[i].z >> 6) * NCRSTLS];
        atomicAdd(sino + aw +
                      NSANGLES * NSBINS * c_r2s[(lm[i].y & 0x3f) + (lm[i].z & 0x3f) * NRNGS],
                  1);
        ...
        // projection views
        short wi = aw / NSANGLES;
        short ai = aw - wi * NSANGLES;
        short a0 = ai == 0 || ai == 223;
        short a126 = ai == 112 || ai == 111;
        if ((a0 || a126) && (itag < MXNITAG)) {
          atomicAdd(pview + (itag >> VTIME) * SEG0 * NSBINS + ssri * NSBINS + wi,
                    Nevnt << (a126 * 8));
        }
...
void CUDART_CB MyCallback(cudaStream_t stream, cudaError_t status, void *data) {
  ...
  if (nchnkrd < lmprop.nchnk) {
    ...
    hstatus = H5Dread(h5set.dset, h5set.dtype, h5set.memspace, h5set.dspace, H5P_DEFAULT,
                      (void *)&lmbuff[i * ELECHNK * lmprop.bpe]);
    if (hstatus < 0) {
...
void gpu_hst(LMprop _lmprop, unsigned int *d_rprmt, unsigned int *d_mass, unsigned int *d_pview,
  ...
  cudaMemcpyToSymbol(c_r2s, r2s, NRNGS * NRNGS * sizeof(short));

  // allocate mem for the list mode file
  ushort3 *d_lmbuff;
  HANDLE_ERROR(
      cudaMallocHost((void **)&lmbuff, NSTREAMS * ELECHNK * (size_t)lmprop.bpe)); // host pinned
  HANDLE_ERROR(cudaMalloc((void **)&d_lmbuff, NSTREAMS * ELECHNK * sizeof(ushort3))); // device
  ...
  for (int n = 0; n < lmprop.nchnk; n++) { //
    ...
    HANDLE_ERROR(cudaMemcpyAsync(&d_lmbuff[si * ELECHNK], &lm[si * ELECHNK * 3],
    ...
    hst<<<BTHREADS, NTHREADS, 0, stream[si]>>>(d_lmbuff, d_rprmt, d_mass, d_pview, d_sino, d_c2s,
                                               ...
                                               si * ELECHNK, lmprop.tstart, lmprop.tstop);
