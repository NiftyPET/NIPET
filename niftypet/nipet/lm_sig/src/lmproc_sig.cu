...
H5setup initHDF5(H5setup h5set, char *fname, hsize_t bytes) {
  ...
  h5set.dset = H5Dopen(h5set.file, LMDATASET, H5P_DEFAULT);
  if (h5set.dset < 0) {
...
void lmproc(hstout hout, LMprop lmprop, unsigned short *frames, int nfrm, short *r2s, int *c2s,
  ...
  if (lmprop.nitag > MXNITAG) {
    pve = MXNITAG / (1 << VTIME) * SEG0 * NSBINS;
    ...
    pve = lmprop.nitag / (1 << VTIME) * SEG0 * NSBINS;
    HANDLE_ERROR(cudaMalloc(&d_pview, (lmprop.nitag / (1 << VTIME)) * SEG0 * NSBINS *
                                          sizeof(unsigned int)));
    HANDLE_ERROR(cudaMemset(
        d_pview, 0, (lmprop.nitag / (1 << VTIME)) * SEG0 * NSBINS * sizeof(unsigned int)));
  }
  ...
  //---sinogram
  int tot_bins = NSANGLES * NSBINS * NRNGS * NRNGS;
  unsigned int *d_sino;
  if (nfrm == 1) {
  ...
  // LUTs
  int *d_c2s;
  HANDLE_ERROR(cudaMallocManaged(&d_c2s, NCRSTLS * NCRSTLS * sizeof(int)));
  HANDLE_ERROR(cudaMemcpy(d_c2s, c2s, NCRSTLS * NCRSTLS * sizeof(int), cudaMemcpyHostToDevice));
