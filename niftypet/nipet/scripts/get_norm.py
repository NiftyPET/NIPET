...
for nk in rawh5['r4']:
    if nk in rawh5['r5'] and nk != 'fh5':
        ...
        if diff > 1e-5:
            raise ValueError("Difference in normalisation components in the two HDF5 norm files"
                             f" ({nk}: {diff})")
...
for li in range(len(axLUT['li2sn'])):
    ...
    if sni not in range(1, Cnt['NSEG0'], 2):
        ...
        nrmsn[sni] = 0.25 * (nrmsn[sni] +
                             (geosn[sni] * np.reshape(effsn, (Cnt['NSBINS'], Cnt['NSANGLES'])).T))
nrm_ = nrmsn.transpose(1, 0, 2)
# si=23
# plt.matshow(100*(nrm_[:,si,:]-nrm[:,si,:])/nrm[:,si,:])
# plt.colorbar()

if (fnrm := opth / f'nrm_{li + 1}.npz').exists():
